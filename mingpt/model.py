...
结构：Token/Position Embedding → N × Transformer Block → LayerNorm → LM Head
每个 Block = LayerNorm → Causal Self-Attention → 残差 → LayerNorm → MLP → 残差

参考：OpenAI GPT-2、HuggingFace GPT2 实现。
"""
...
class CausalSelfAttention(nn.Module):
    """
    因果（单向）多头自注意力：每个位置只能看到它及之前的 token。
    通过下三角 mask 实现。
    """
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, train_config):
        """
        AdamW：Linear 的 weight 做 weight decay；bias、LayerNorm、Embedding 不做 decay。
        """
        decay = set()
        no_decay = set()
